...
1/ Create Cluster Root Certification Authority (stored as an Opensvc secret)
    ...
    om system/sec/ca-${OSVC_CLUSTER_NAME} set --kw o=PyconFR2019 --kw c=fr --kw email=${OSVC_CLUSTER_NAME}-admin@pycon.fr
...
2/ Create Cluster listener certificate (will be presented to remote user)
...
3/ Deploy vip service (used by end user to connect the cluster)
    ...
    om cluster print config  # check new config snippet
    om mon -w    # check service live deployment
    ping -c1 c${OSVC_CLUSTER_NUMBER}vip
    ip addr show br-prd      # on cXn1

4/ Test vip service failover
    om system/svc/vip switch
    om mon -w    # check service live relocation to other node
    om system/svc/vip giveback
    ping -c1 c${OSVC_CLUSTER_NUMBER}vip
    ip addr show br-prd      # on cXn2
